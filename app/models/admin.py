...
class User(BaseModel, TimestampMixin):
    ...
    is_patientuser = fields.BooleanField(default=False, description="是否为患者用户", index=True)
    last_login = fields.DatetimeField(null=True, description="最后登录时间", index=True)
    roles = fields.ManyToManyField("models.Role", related_name="user_roles")
    dept_id = fields.IntField(null=True, description="部门ID", index=True)
    theme = fields.CharField(max_length=20, null=True, description="主题颜色", index=True)
    logo_type = fields.CharField(max_length=20, null=True, description="Logo类型", index=True)
...
class Dept(BaseModel, TimestampMixin):
    ...
    leader = fields.CharField(max_length=64, null=True, description="部门负责人")
